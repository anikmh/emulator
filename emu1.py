...
from sklearn.gaussian_process.kernels import RBF, DotProduct, RationalQuadratic, Matern, WhiteKernel
...
class modGPR:
    ...
    def read_data(self, file_list, ds_name, nd_in, param_list):
        ...
        for hdf_file in file_list:
            ...
            print("Pymodule : Table in file ", hdf_file, " has lines ", new_data.get_nlines())
            ...
            if new_data.is_column("emulated"):
                if new_data.is_column("mult"):
                    new_data.delete_rows_func("emulated>0.5 || abs(mult)<0.5")
                else:
                    new_data.delete_rows_func("emulated>0.5")
            else:
                if new_data.is_column("mult"):
                    new_data.delete_rows_func("abs(mult)<0.5")
    ...
    def modTrain(self):
        print("Pymodule : table now has ", self.train_table.get_nlines(), " lines.")
        ...
        for i in self.input_list:
            train_data.append(np.array(self.train_table[i][0:self.train_table.get_nlines()]))
        ...
        for i in self.output_list:
            output_data.append(np.array(self.train_table[i][0:self.train_table.get_nlines()]))
        ...
        self.gpr = GPR(kernel=self.kernel, random_state=0, n_restarts_optimizer= 5, 
...
class modDnn:
    ...
    def read_data(self, hdf_file, ds_name, nd_in, param_list):

        input_list = []
        output_list = []
        for i in range(0, nd_in):
            input_list.append(param_list[i])

        for i in range(nd_in, len(param_list)):
            output_list.append(param_list[i])

        train_file = h5py.File(hdf_file, 'r')
        data = train_file[ds_name+'/data/']

        input_data = []
        for i in input_list:
            input_data.append(np.array(data[str(i)]))
        input_data = np.asarray(input_data)
        input_data = input_data.transpose()
        self.input_data = self.SS1.fit_transform(input_data)

        output_data = []
        for i in output_list:
            output_data.append(np.array(data[str(i)]))
        output_data = np.asarray(output_data)
        output_data = output_data.transpose()
        self.output_data = self.SS2.fit_transform(output_data)

        print("PyModule 3: input data: ", np.shape(self.input_data))
        print("PyModule 3: output data: ", np.shape(self.output_data))

    def modTrain(self):

        print("Pymodule: starting train")
        x_train, x_test, y_train, y_test = train_test_split(
            self.input_data, self.output_data, test_size=0.15)
        print("PyModule : Training array : ", x_train.shape)
        print("PyModule : Target array : ", y_train.shape)
        ...
        model = tf.keras.Sequential(
            [
                tf.keras.layers.Dense(
                    302, input_shape=(151,), activation='tanh'),
    ...
    def predict(self, trial_input):
        ...
        print("PyModule : starting predict")
        trial_input = np.asarray(trial_input)
        trial_input = trial_input.reshape(1, 151)
        trial_input = self.SS1.transform(trial_input)
